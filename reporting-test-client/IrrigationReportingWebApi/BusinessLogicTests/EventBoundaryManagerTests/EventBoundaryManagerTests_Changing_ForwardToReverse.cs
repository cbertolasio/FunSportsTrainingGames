...
{
	[TestFixture]
	public class EventBoundaryManagerTests_Changing_ForwardToReverse
	{
		/// <summary>
		...
		{
			var testData = MultiEventBoundaryTestData.GetForwardToReverseData().ToArray();
		...
		{
			var testData = MultiEventBoundaryTestData.GetForwardToReverseData().ToList();
		...
		public void GetEventBoundaries_Sets_Expected_DegreesOfTravel_For_The_LastBoundary()
		{
			...
			var testData = MultiEventBoundaryTestData.GetForwardToReverseData();
			...
			Assert.AreEqual(130.0, expected, "PRECON");
